...
export default Ember.Component.extend({
  ...
  jsonInitialValue: Ember.computed('initialValue', function() {
    ...
    return Ember.typeOf(initialValue) === 'string' && !Ember.isBlank(initialValue)
  ...
  jsonValue: Ember.computed('value', function() {
    ...
    return Ember.typeOf(value) === 'string' && !Ember.isBlank(value)
      ? JSON.parse(value)
      : null;
  }),
  ...
  selectedFile: Ember.computed('uploadData', function() {
    ...
    return uploadData && uploadData.files && uploadData.files.length > 0
      ? uploadData.files[0]
      : null;
  }),
  ...
  init: function() {
    ...
    var fileComponentConfig = Ember.get(applicationConfig, 'APP.components.fileComponent') || {};
    ...
    initProperty({ propertyName: 'uploadUrl', defaultPropertyValue: null });
    initProperty({ propertyName: 'downloadUrl', defaultPropertyValue: null });
    initProperty({ propertyName: 'maxUploadFileSize', defaultPropertyValue: null });
    initProperty({ propertyName: 'placeholder', defaultPropertyValue: '(no file)' });
    initProperty({ propertyName: 'uploadOnModelPreSave', defaultPropertyValue: true });
    initProperty({ propertyName: 'showUploadButton', defaultPropertyValue: false });
    initProperty({ propertyName: 'showModalDialogOnUploadError', defaultPropertyValue: false });
    initProperty({ propertyName: 'showModalDialogOnDownloadError', defaultPropertyValue: true });
    initProperty({ propertyName: 'addButtonTitle', defaultPropertyValue: 'Add file' });
    initProperty({ propertyName: 'removeButtonTitle', defaultPropertyValue: 'Remove file' });
    initProperty({ propertyName: 'uploadButtonTitle', defaultPropertyValue: 'Upload file' });
    initProperty({ propertyName: 'downloadButtonTitle', defaultPropertyValue: 'Download file' });
  ...
  didInsertElement: function() {
    ...
    var fileInputId = _this.get('elementId') + 'FileComponent';
    var fileInput = _this.$('.file-component-file-input');
    fileInput.attr('id', fileInputId);

    // Add button is <label>, so click on it will fire click on <input> to which points "for" attribute.
    var addFileButton = _this.$('.file-component-add-button');
    addFileButton.attr('for', fileInputId);

    var removeFileButton = _this.$('.file-component-remove-button');
    removeFileButton.on('click', function() {
      _this.removeFile.call(_this, null);
    });

    var uploadFileButton = _this.$('.file-component-upload-button');
    uploadFileButton.on('click', function() {
      _this.uploadFile.call(_this, null);
    });

    var downloadFileButton = _this.$('.file-component-download-button');
    ...
    var errorModalDialog = _this.$('.file-component-error-modal-dialog');
    ...
    var onFileAdd = function(e, uploadData) {
      var selectedFile = uploadData && uploadData.files && uploadData.files.length > 0
        ? uploadData.files[0]
        : null;
  ...
  willDestroyElement: function() {
    var fileInput = this.$('.file-component-file-input');
  ...
  uploadFile: function() {
    ...
    return new Ember.RSVP.Promise(function(resolve, reject) {
      ...
      uploadData.submit().done(function(result, textStatus, jqXhr) {
        ...
        _this.sendAction('uploadSuccess', { uploadData: uploadData, response: jqXhr });
        ...
        _this.sendAction('uploadFail', { uploadData: uploadData, response: jqXhr });
    ...
    return new Ember.RSVP.Promise(function(resolve, reject) {
      ...
      Ember.$.fileDownload(_this.get('downloadUrl') + '?' + Ember.$.param(jsonInitialValue), {
        successCallback: function(url) {
          _this.sendAction('downloadSuccess', { downloadData: jsonInitialValue, response: 'success' });
        ...
        failCallback: function(errorText, url) {
          ...
          _this.sendAction('downloadFail', { downloadData: jsonInitialValue, response: errorText });
          reject(errorText);
          _this.set('downloadIsInProgress', false);
        }
      });
