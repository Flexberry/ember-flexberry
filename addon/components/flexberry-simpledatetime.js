...
export default FlexberryBaseComponent.extend({
  ...
  */
  _validationDateTime() {
    ...
    let inputValue = $('.custom-flatpickr', this.element)[0].value;
  ...
  _flatpickrCreate() {
    ...
    const options = {
      ...
      onClose: () => {
        ...
        $('.custom-flatpickr', this.element).blur();
      },
    };

    this.set('_flatpickr', $('.flatpickr > input', this.element).flatpickr(options));
    ...
    $('.custom-flatpickr', this.element).mask(timeless ? this.dateMask : this.dateTimeMask);

    $('.custom-flatpickr', this.element).change($.proxy(function () {
      this._onChange();
    }, this));

    $('.custom-flatpickr', this.element).keydown($.proxy(function (e) {
      if (e.which === 13) {
        $('.custom-flatpickr', this.element).blur();
        this._onChange();
      }
    ...
    $('.custom-flatpickr', this.element).prop('readonly', this.get('readonly'));

    let namespace = this.elementId;
    this.set('eventNamespace', namespace);
    $(document).on(`mousedown.${namespace}`, (e) => {
      let clicky = $(e.target);
      if (clicky.closest('.flatpickr-calendar').length === 0 && clicky.get(0) !== $('.custom-flatpickr', this.element).get(0)) {
