...
export default Mixin.create({
  ...
  */
  ...
  /**
    ...
    @readOnly
  */
  voiceTypingIsActive: computed('isUseVoiceTyping', function() {
    const appConfig = getOwner(this).factoryFor('config:environment').class;
    const useVoiceTypingInAllControls = get(appConfig, 'APP.useVoiceTypingInAllControls');
    const isUseVoiceTyping = this.get('isUseVoiceTyping');
    if (!isNone(isUseVoiceTyping)) {
  ...
  */
  ...
  init() {
    ...
    if (this.get('voiceTypingIsActive')) {
      ...
      if (!isNone(window.SpeechRecognition)) {
