...
executeTest('check filter on toolbar with filter projection', async (store, assert, app) => {
  assert.expect(5);
  const path = 'components-acceptance-tests/flexberry-objectlistview/custom-filter';
  ...
  await fillIn($filterInput, adressEtalone);
  ...
  await  fillIn($filterInput, typeNameEtalone);
    
  await refreshListByFunction(refreshFunction, controller);
  await settled();
  currentModel = get(controller, 'model');
