...
executeTest('check filter renders', (store, assert) => {
  ...
  Ember.run(() => {
    ...
    andThen(function() {
      ...
      filterOperation.click();

      let filterMenu = Ember.$('.menu.animating').children()[0];
      filterMenu.click();

      let firstObjectFilterValue= Ember.$('.ember-text-field')[0];
      firstObjectFilterValue.value = 'Value that will never enter';
