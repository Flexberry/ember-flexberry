...
export default BaseEditFormController.extend({
  ...
  /**
   ...
   @type String
   @default 'pseudodetailes-list'
  */
  parentRoute: 'integration-examples/pseudodetailes/pseudodetailes-list',

  getCellComponent: null,

  votes: Ember.computed('model.id', function() {
    let authorId = this.get('model.id');
    let store = this.get('store');
    let modelName = 'ember-flexberry-dummy-vote';

    let builder = new Builder(store)
    .from(modelName)
    .selectByProjection('VoteE')
    .where('author', FilterOperator.Eq, authorId);
