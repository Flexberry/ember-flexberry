...
module( 'Integration | Component | flexberry-tab-bar', function(hooks) {
  ...
  test('tabs are switched in menu and in dropdown list', async function(assert) {
    assert.expect(4);
    ...
    // To select some item, menu must contain such item (with the specified caption).
    $itemMenu = $('.item:contains(' + this.get('items')[2].caption + ')', $menu);
    $itemDropdown = $('.item:contains(' + this.get('items')[2].caption + ')', $dropdown);
