...
moduleFor('transform:enum-string', 'Unit | Transform | enum-string', {
...
test('it serializes existing values from array', function(assert) {
  ...
  serializeValues(assert, arrayEnum, this, existingValue, existingValue);
});

test('it deserializes existing values from array', function(assert) {
  var existingValue = arrayEnum[0];
  deserializeValues(assert, arrayEnum, this, existingValue, existingValue);
});

test('it serializes existing values from object', function(assert) {
  var existingValue = 'Пусто';
  serializeValues(assert, objectEnum, this, existingValue, existingValue);
});

test('it deserializes existing values from object', function(assert) {
  var existingValue = 'Пусто';
  deserializeValues(assert, objectEnum, this, existingValue, existingValue);
});
...
test('it throws error on serializing nonexisting values from array', function(assert) {
  ...
  assert.throws(
    function() {
      serializeValues(assert, arrayEnum, this, nonExistingValue, nonExistingValue);
    },
    Error
  );
});
...
test('it throws error on serializing nonexisting values from object', function(assert) {
  ...
  assert.throws(
    function() {
      serializeValues(assert, objectEnum, this, nonExistingValue, nonExistingValue);
    },
    Error
  );
});
